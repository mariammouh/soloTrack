                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <div class=" d-flex align-items-center navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4 ">
                                <a href="{{ route('devis.add', $autoentrepreneur->id) }}" class="add-button-container">+
                                    Ajouter devis</a>
                            ...
                            @if (!empty($devis))
                                ...
                                <tbody>
                                    @foreach ($devis as $devi)
                                        <tr class="TableRow">
                                            <td>
                                                <div class="d-flex px-2 py-1">
                                                    <div>
                                                        <a href="{{ route('devis.download.pdf', $devi->id_devis) }}">
                                                            <img src="../assets/img/download-pdf.png"
                                                                class="avatar avatar-sm me-3">
                                                        </a>
                                                    </div>
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm">{{ $devi->client_nom }}</h6>
                                                        <p class="text-xs text-secondary mb-0">
                                                            {{ $devi->client_adresse }}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex flex-column justify-content-center">
                                                    @if (!empty($ProduitsChaqueDevis))
                                                        @foreach ($ProduitsChaqueDevis as $produit)
                                                            @if ($produit->id_devis == $devi->id_devis)
                                            ...
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $devi->quantite }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $devi->mode_paiement }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $devi->date_operation }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $devi->tva }}%</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $devi->total_prix }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <button class="editBtn"> <svg height="1em" viewBox="0 0 512 512">
                                                        <path
                                                ...
                                                <form method="post" action="{{ route('devis.destroy', $devi->id) }}"
                                                    class="p-0">
                                                    @csrf
                                                    @method('delete')
                                        ...
                                        <tr class="UpdateRow hidden-row">
                                            <form method="post" action="{{ route('devis.update', $devi->id_devis) }}"
                                                class="p-6">
                                                @csrf
                                                @method('patch')
                                                <td>
                                                        ...
                                                        <h6 class="mb-0 text-sm">
                                                            <select name="client" id="clientSelect" class="form-select">
                                                                @if ($clients)
                                                                    ...
                                                                    @endforeach
                                                                @endif
                                                                <option value="" id="ShowClientInput">Nouveau client
                                                                </option>
                                                    ...
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm">
                                                            <div class="d-flex flex-column justify-content-center">
                                                                @if (!empty($ProduitsChaqueDevis))
                                                                    @foreach ($ProduitsChaqueDevis as $produit)
                                                                        @if ($produit->id_devis == $devi->id_devis)
                                                                            <h6 class="mb-0 text-sm">
                                                                                {{ $produit->produit_nom }}</h6>
                                                                            <p class="text-xs text-secondary mb-0">Prix
                                                                                unitaire:
                                                                                {{ $produit->produit_prix }}</p>
                                                                            <p class="text-xs text-secondary mb-0">
                                                                                Quantite:
                                                            ...
                                                            <select name="produit" id="ProduitSelect"
                                                                ...
                                                                <option value="0" id="ShowProduitInput">
                                                                </option>
                                                ...
                                                <td class="align-middle text-center">
                                                    <input type="text" class="form-control" name="mode_paiement"
                                                        value="{{ $devi->mode_paiement }}">
                                                </td>
                                                <td class="align-middle text-center">
                                                    <input type="date" class="form-control" name="date_operation"
                                                        value="{{ $devi->date_operation }}">
                                                </td>
                                                <td class="align-middle text-center">
                                                    <input type="hidden" class="form-control" name="tva"
                                                        value="{{ $devi->tva }}">
                                                </td>
                                                <td class="align-middle text-center">
                                                    <input type="hidden" name="total_prix" class="text-secondary text-xs font-weight-bold"
                                                        value="{{ $devi->total_prix }}">{{ $devi->total_prix }}</input>
                                                </td>
                                                <td class="align-middle text-center text-sm SaveOperation">
                                                    <button class="badge badge-sm bg-gradient-success"
                                                        type="submit">Enregistrer</button>
                                                </td>
                                                <td class="align-middle">
                                                    <button class="bin-button hideUpdateRow">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 39 7" class="bin-top">
                                                            <line stroke-width="4" stroke="white" y2="5"
                                                                x2="39" y1="5"></line>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.editBtn').forEach(button => {
                button.addEventListener('click', function() {
                    const tableRow = this.closest('tr.TableRow');
                    const updateRow = tableRow.nextElementSibling;
                    if (updateRow && updateRow.classList.contains('UpdateRow')) {
            ...
            document.querySelectorAll('.hideUpdateRow').forEach(button => {
                button.addEventListener('click', function() {
                    const updateRow = this.closest('tr.UpdateRow');
                    const tableRow = updateRow.previousElementSibling;
                    if (tableRow && tableRow.classList.contains('TableRow')) {
